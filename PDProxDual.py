...
def f(w, alpha):
    ...
    for i in range(n):
        ...
        xi = X[i]
        alphai = alpha[i]
        sum = sum + hingeloss(alphai, w, xi, yi)
    return 1./n * sum
    
def hingeloss(alpha, w, xi, yi):
    return alpha * (1 - yi*(w.dot(xi.T)))
...
def gradientw(w0,alpha0):
    ...
    for i in range(n):
        arr[i] = alpha0[i] * y[i]
    return -1./n * (X.T.dot(arr))
    
    
def g(w):
    return linalg.norm(w,2)
